...
public class RepairGenerator {
    ...
    private static final Logger logger = LogManager.getLogger(RepairGenerator.class);
    ...
    public static void processRepairs(
        ...
        SailRepository shapesRepo = new SailRepository(shaclSail);
        shapesRepo.init();
        try (RepositoryConnection connection = shapesRepo.getConnection()) {
            connection.add(shapesModel, RDF4J.SHACL_SHAPE_GRAPH);
        }

        List<ContextWithShapes> contextWithShapes = shaclSail.getShapes(
                new IRI[] {RDF4J.SHACL_SHAPE_GRAPH}, true);

        List<Shape> shapes = new ArrayList();
        for (ContextWithShapes shape : contextWithShapes) {
    ...
    public static String processPathExpression(String shapeName, Path path) {
        ...
        if (path instanceof SimplePath) {
            ...
            if (!(((OneOrMorePath) path).getPath() instanceof SimplePath)) {
                throw new RuntimeException("one or more path contains not supported element " + path.getClass().getSimpleName());
            }

            String pathName = getPathName(path);
            createPropertyRules(pathName);
            createOneOrMorePathRules(shapeName, pathName);

            String rule = shapeName + "_st_(X,Y,\"t*\"):-" + shapeName + "_(X,_)" + "," + pathName + "_plus_(X,Y,\"t*\") .\n";
            RepairData.get().getAnnotationRules().add(rule);

            rule = shapeName + "_st_(X,Y,\"t**\"):-" + pathName + "_plus_(X,Y,\"t**\")," + shapeName + "_st_(X,Y,\"t*\"),not " + shapeName + "_st_(X,Y,\"f\") .\n";
            RepairData.get().getInterpretationRules().add(rule);

        } else if (path instanceof ZeroOrMorePath) {

            if (!(((ZeroOrMorePath) path).getPath() instanceof SimplePath)) {
                throw new RuntimeException("zero or more path contains not supported element " + path.getClass().getSimpleName());
            }

            String pathName = getPathName(path);
            createPropertyRules(pathName);
            createZeroOrMorePathRules(shapeName, pathName);

            String rule = shapeName + "_st_(X,Y,\"t*\"):-" + shapeName + "_(X,_)" + "," + pathName + "_star_(X,Y,\"t*\") .\n";
            RepairData.get().getAnnotationRules().add(rule);

            // reflexive match
            rule = pathName + "_star_(X,X,\"t*\"):-" + shapeName + "_(X,_) .\n";
            RepairData.get().getAnnotationRules().add(rule);

            rule = shapeName + "_st_(X,Y,\"t**\"):-" + pathName + "_star_(X,Y,\"t**\")," + shapeName + "_st_(X,Y,\"t*\"),not " + shapeName + "_st_(X,Y,\"f\") .\n";
            RepairData.get().getInterpretationRules().add(rule);
            ...
            for (Path pathEl : seq.getSequence()) {
                if (pathEl instanceof SimplePath || pathEl instanceof InversePath
                        || pathEl instanceof OneOrMorePath || pathEl instanceof ZeroOrMorePath) {
                ...
                if (pathEl instanceof OneOrMorePath) {
                    if (!(((OneOrMorePath) pathEl).getPath() instanceof SimplePath)) {
                        throw new RuntimeException("one or more path contains not supported element " + pathEl.getClass().getSimpleName());
                    }
                    createOneOrMorePathRules(shapeName, getPathName(pathEl));
                }

                if (pathEl instanceof ZeroOrMorePath) {
                    if (!(((ZeroOrMorePath) pathEl).getPath() instanceof SimplePath)) {
                        throw new RuntimeException("zero or more path contains not supported element " + pathEl.getClass().getSimpleName());
                    }
                    createZeroOrMorePathRules(shapeName, getPathName(pathEl));
                }
            }
            ...
            for (Path pathEl : seq.getSequence()) {
                ...
                } else if (pathEl instanceof OneOrMorePath) {
                    String pathName = getPathName(pathEl);
                    rule += "," + pathName +
                            "_plus_(X" + (count >= 0 ? count : "") + ",X" + (count + 1) + ",\"t*\")";
                } else if (pathEl instanceof ZeroOrMorePath) {
                    String pathName = getPathName(pathEl);

                    // reflexice match
                    String currentX = "" + (count >= 0 ? count : "");
                    String zeroMatchRule = pathName +
                            "_star_(X" + currentX + ",X" + currentX + ",\"t*\"):-" + rule + " .\n";
                    RepairData.get().getAnnotationRules().add(zeroMatchRule);

                    rule += "," + pathName +
                            "_star_(X" + (count >= 0 ? count : "") + ",X" + (count + 1) + ",\"t*\")";
                } else {
                    throw new RuntimeException("sequence path contains not supported element " + pathEl.getClass().getSimpleName());
                }
                count++;
    ...
    private static void getPropertyPathCutRule(String shapeName, Path path, String st) {

        if (path instanceof SimplePath) {

            RepairData.get().getRepairRules().add(
                    ns(nss, path.getId()) + "_(X,Y,\"f\"):-" + ns(nss, path.getId()) + "_(X,Y,\"t*\")," + st + "_(X,Y,\"f\") .\n");
            ...
            RepairData.get().getRepairRules().add(
                    pathName + "_(Y,X,\"f\"):-" + pathName + "_inv_(X,Y,\"t*\")," + st + "_(X,Y,\"f\") .\n");

        } else if (path instanceof OneOrMorePath) {

            if (!(((OneOrMorePath) path).getPath() instanceof SimplePath)) {
                throw new RuntimeException("one or more path contains not supported element " + ((OneOrMorePath) path).getPath().getClass().getSimpleName());
            }

            OneOrMorePath star = (OneOrMorePath) path;
            String pathName = ns(nss, star.getPath().getId());

            RepairData.get().getRepairRules().add(
                    "1 {" + pathName + "_plus_(X,Y,\"f\"): "  + st + "_(X,Y,\"f\")," + pathName + "_plus_(X,Y,\"t*\")} 1:-" + st + "_(X,Y,\"f\")" + " .\n");

        } else if (path instanceof ZeroOrMorePath) {

            if (!(((ZeroOrMorePath) path).getPath() instanceof SimplePath)) {
                throw new RuntimeException("zero or more path contains not supported element " + ((ZeroOrMorePath) path).getPath().getClass().getSimpleName());
            }

            ZeroOrMorePath star = (ZeroOrMorePath) path;
            String pathName = ns(nss, star.getPath().getId());

            RepairData.get().getRepairRules().add(
                    "1 {" + pathName + "_star_(X,Y,\"f\"): "  + st + "_(X,Y,\"f\")," + pathName + "_star_(X,Y,\"t*\")} 1:-" + st + "_(X,Y,\"f\")" + " .\n");
            ...
            for (Path pathEl : ((SequencePath) path).getSequence()) {
                if (pathEl instanceof SimplePath) {
                    ...
                    body += getPathName(pathEl) +
                            "_plus_(X" + (count >= 0 ? count : "") + ",X" + (count + 1) + ",\"t*\"),";
                    head += getPathName(pathEl) +
                            "_plus_(X" + (count >= 0 ? count : "") + ",X" + (count + 1) + ",\"f\");";
                } else if (pathEl instanceof ZeroOrMorePath) {
                    body += getPathName(pathEl) +
                            "_star_(X" + (count >= 0 ? count : "") + ",X" + (count + 1) + ",\"t*\"),";
                    head += getPathName(pathEl) +
                            "_star_(X" + (count >= 0 ? count : "") + ",X" + (count + 1) + ",\"f\");";
                } else {
                    throw new RuntimeException("sequence path contains not supported element " + pathEl.getClass().getSimpleName());
                }
                count++;
            }
            body = body + st + "_(X,X" + count + ",\"f\") .\n";
    ...
    private static String getPathName(Path path) {
        if (path instanceof SimplePath) {
            ...
            return ns(nss, ((OneOrMorePath) path).getPath().getId());
        } else if (path instanceof ZeroOrMorePath) {
            return ns(nss, ((ZeroOrMorePath) path).getPath().getId());
        } else {
    ...
    private static void getCardinalityRules(String shapeName, Path path, String st, long minCount, Collection<ConstraintComponent> valueNodeConstraints) {
        ...
        if (path instanceof SimplePath || path instanceof SequencePath) {
            ...
            if (minCount > 0) {
                ...
                if (!repairChoices.startsWith("choose")) {
            ...
            if (minCount > 0) {
                String repairChoices = "choose(" + shapeName + ",X," + firstPath + "_inv_,0):-" + shapeName + "_(X,\"t*\") .\n";

                for (int i = 1; i <= minCount; i++) {

                    repairChoices = "choose(" + shapeName + ",X," + firstPath + "_inv_," + i + ");" + repairChoices;
            ...
            if (minCount > 0) {
                String repairChoices = "choose(" + shapeName + ",X," + firstPath + "_plus_,0):-" + shapeName + "_(X,\"t*\") .\n";
            ...
            if (SHACLData.get().getConstantsFacts().size() > 0) {
                RepairData.get().getRepairRules().add(
                        "0 {" + firstPath + "_plus_(X,Y,\"t\"):const(Y)} " +
                                SHACLData.get().getConstantsFacts().size() +
                                ":-" + shapeName + "_(X,\"t*\") .\n"
                );
                RepairData.get().getChangeSetRules().add("#minimize { 1@1,Y,X: " + firstPath + "_(Y,X,\"t\"), const(Y) } .\n");
            ...
            if (minCount > 0) {
                ...
                logger.warn("zeroOrMorePath true advise not supported");
    ...
    private static void getUniversalRules(String shapeName, Path path, String st, Set<ConstraintComponent> valueNodeConstraints) {
        ...
        if (path instanceof SimplePath || path instanceof SequencePath) {
            ...
            RepairData.get().getRepairRules().add(
                    firstPath + "_plus_(X,@new(" + shapeName + ",X," + firstPath + "_plus_,1.." + 1 + "),\"t\")" +
                            ":-choose(" + shapeName + ",X," + firstPath + "," + 1 + ") .\n");
            ...
            if (SHACLData.get().getConstantsFacts().size() > 0) {
                ...
                RepairData.get().getChangeSetRules().add("#minimize { 1@1,Y,X: " + firstPath + "_plus_(Y,X,\"t\"), const(Y) } .\n");
            }
            ...
            if (seq.getSequence().get(spI) instanceof SimplePath) {
                ...
                ruleBody +=
                        currentPathName + "_inv_(X" +
                                ((spI-1) >= 0 ? (spI-1) : "") +
                                ",X" + spI + ",\"t**\"),";
            } else if (seq.getSequence().get(spI) instanceof OneOrMorePath) {
                ruleBody +=
                        currentPathName + "_plus_(X" +
                                ((spI-1) >= 0 ? (spI-1) : "") +
                                ",X" + spI + ",\"t**\"),";
            } else if (seq.getSequence().get(spI) instanceof ZeroOrMorePath) {
                ruleBody +=
                        currentPathName + "_star_(X" +
                                ((spI-1) >= 0 ? (spI-1) : "") +
                                ",X" + spI + ",\"t**\"),";
            } else {
    ...
    public static void createSequencePathRules(SequencePath seq, String propertyPathName, long minCount) {
        ...
        for (int pI = 0; pI < seq.getSequence().size(); pI++) {
            ...
            if (seq.getSequence().get(pI) instanceof SimplePath) {
                ...
                postfix = "_inv_";
            } else if (seq.getSequence().get(pI) instanceof OneOrMorePath) {
                postfix = "_plus_";
            ...
            if (pI < (seq.getSequence().size() - 1)) {
                ...
                nextObject = "@new(" + propertyPathName + ",X" + currentPathIndex +"," + nextPath + postfix + "," + 1 + ")";

                repairChoices = "choose(" + propertyPathName + ",X" + currentPathIndex + "," + nextPath + postfix + "," + 1 + ");" +
                        "choose(" + propertyPathName + ",X" + currentPathIndex + "," + nextPath + postfix + ",0):-" + ruleBody;
                RepairData.get().getRepairRules().add(repairChoices);

                String repairChoice = nextPath + postfix + "(X" + currentPathIndex + "," + nextObject + ",\"t\"):-"
                        + "choose(" + propertyPathName + ",X" + currentPathIndex + "," + nextPath + postfix + "," + 1 + "),"
                        + ruleBody;
                ...
                String repairChoice = "0 {" + nextPath + postfix + "(X" + currentPathIndex + "," + nextObject + ",\"t\")} 1:-"
                        + ruleBody;
                RepairData.get().getRepairRules().add(repairChoice);
    ...
    private static void createOneOrMorePathRules(String shapeName, String pathName) {
        ...
        for (int i = 1; i <= maxPathLength; i++) {
            ...
            repairChoices = "chooseOneOrMore(" + pathName + ",X," + i + ",1,Y)" + repairChoices;
            if (i < maxPathLength) {
            ...
            for (int j = 0; j < i; j++) {
                annotationRule += pathName + "_(X" + (j==0? "" : (j-1)) + "," + ((j+1)==i ? "Y" : "X" + j) + ",\"t*\"),";
                interpretationRule += "," + pathName + "_(X" + (j==0? "" : (j-1)) + "," + ((j+1)==i ? "Y" : "X" + j) + ",\"t**\")";
            }
            annotationRule = annotationRule.substring(0, annotationRule.length() - 1) + " .\n";
            interpretationRule += " .\n";
    ...
    private static void createZeroOrMorePathRules(String shapeName, String pathName) {

        logger.warn("zeroOrMorePath true advise not supported");

        RepairData.get().getProgramConstraints().add(
                ":-" + pathName + "_star_(X,Y,\"t\") .\n");

        RepairData.get().getAnnotationRules().add(
                pathName + "_star_(X,Y,\"t*\"):-" + pathName + "_(X,Y,\"t*\") .\n");
        ...
        RepairData.get().getAnnotationRules().add(
                pathName + "_star_(X,Y,\"t*\"):-" +
                        pathName + "_star_(X,X0,\"t*\")," + pathName + "_(X0,Y,\"t*\") .\n");

        RepairData.get().getInterpretationRules().add(
                pathName + "_star_(X,Y,\"t**\"):-" +
                        pathName + "_star_(X,Y,\"t*\"),not " + pathName + "_star_(X,Y,\"f\")," +
                        pathName + "_(X,Y,\"t**\") .\n");
        ...
        RepairData.get().getRepairRules().add(
                pathName + "_(X1,Y,\"f\");" + pathName + "_star_(X,X1,\"f\"):-" +
                        pathName + "_star_(X,Y,\"f\")," + pathName + "_(X1,Y,\"t*\")," + pathName + "_star_(X,X1,\"t*\") .\n");

        RepairData.get().getRepairRules().add(
                pathName + "_(X,Y,\"f\"):-" +
                        pathName + "_star_(X,Y,\"f\")," + pathName + "_(X,Y,\"t*\") .\n");

        // reflexive match cannot be falsified
        RepairData.get().getProgramConstraints().add(":-" + pathName + "_star_(X,X,\"f\") .\n");
    }
    ...
    private static void getEqualsRules(String shapeName, String st, Path path, String equalsName) {
        ...
        if (path instanceof SequencePath) {
            ...
            RepairData.get().getRepairRules().add(equalsName + "_(X,Y,\"t\");" + st + "_(X,Y,\"f\"):-" +
                    shapeName + "_(X,\"t*\")," + st + "_(X,Y,\"t*\") .\n");
            RepairData.get().getRepairRules().add(st + "_(X,Y,\"t\");" + equalsName + "_(X,Y,\"f\"):-" +
                    shapeName + "_(X,\"t*\")," + equalsName + "_(X,Y,\"t*\") .\n");

            RepairData.get().getRepairRules().add(
                    "1 {" + st + "_(X,Y,\"f\"):" + st + "_(X,Y,\"t*\")," + equalsName + "_(X,Y,\"t*\")" +
                            ";" + equalsName + "_(X,Y,\"f\"):" + st + "_(X,Y,\"t*\")," + equalsName + "_(X,Y,\"t*\")} 1:-" +
                    shapeName + "_(X,\"f\") .\n");

            RepairData.get().getProgramConstraints().add(":-" + shapeName + "_(X,\"t*\")," +
                    equalsName + "_(X,Y,\"t**\"),not " + st + "_(X,Y,\"t**\") .\n");
            RepairData.get().getProgramConstraints().add(":-" + shapeName + "_(X,\"t*\")," +
                    st + "_(X,Y,\"t**\"),not " + equalsName + "_(X,Y,\"t**\") .\n");

        } else if (path instanceof SimplePath) {

            String property = ns(nss, path.getId());

            RepairData.get().getRepairRules().add(equalsName + "_(X,Y,\"t\");" + property + "_(X,Y,\"f\"):-" +
                    shapeName + "_(X,\"t*\")," + property + "_(X,Y,\"t*\") .\n");
            RepairData.get().getRepairRules().add(property + "_(X,Y,\"t\");" + equalsName + "_(X,Y,\"f\"):-" +
                    shapeName + "_(X,\"t*\")," + equalsName + "_(X,Y,\"t*\") .\n");

            RepairData.get().getRepairRules().add("notEquals(" + shapeName + ",X," + property + "," + equalsName + "):-" +
                    shapeName + "_(X,\"f\")," + property + "_(X,Y,\"t*\"),not " + equalsName + "_(X,Y,\"t*\") .\n");
            RepairData.get().getRepairRules().add("notEquals(" + shapeName + ",X," + property + "," + equalsName + "):-" +
                    shapeName + "_(X,\"f\"),not " + property + "_(X,Y,\"t*\")," + equalsName + "_(X,Y,\"t*\") .\n");

            RepairData.get().getRepairRules().add(
                    "1 {" + property + "_(X,Y,\"f\"):" + equalsName + "_(X,Y,\"t*\");" +
                            equalsName + "_(X,Y,\"f\"):" + property + "_(X,Y,\"t*\");" +
                            property + "_(X,@new(" + shapeName + ",X," + property + ",1),\"t*\");" +
                            equalsName + "_(X,@new(" + shapeName + ",X," + equalsName + ",1),\"t*\")" +
                            "} 1:-" +
                            shapeName + "_(X,\"f\"),not notEquals(" + shapeName + ",X," + property + "," + equalsName + ") .\n");

            RepairData.get().getProgramConstraints().add(":-" + shapeName + "_(X,\"t*\")," +
                    equalsName + "_(X,Y,\"t**\"),not " + property + "_(X,Y,\"t**\") .\n");
            RepairData.get().getProgramConstraints().add(":-" + shapeName + "_(X,\"t*\")," +
                    property + "_(X,Y,\"t**\"),not " + equalsName + "_(X,Y,\"t**\") .\n");
            ...
            RepairData.get().getRepairRules().add(
                    "1 {" + property + "_(X,Y,\"f\"):" + property + "_(X,Y,\"t*\")," + equalsName + "_(Y,X,\"t*\")" +
                            ";" + equalsName + "_(Y,X,\"f\"):" + property + "_(X,Y,\"t*\")," + equalsName + "_(Y,X,\"t*\")} 1:-" +
                            shapeName + "_(X,\"f\") .\n");

            RepairData.get().getProgramConstraints().add(":-" + shapeName + "_(X,\"t*\")," +
                    equalsName + "_(Y,X,\"t**\"),not " + st + "_(X,Y,\"t**\") .\n");
            RepairData.get().getProgramConstraints().add(":-" + shapeName + "_(X,\"t*\")," +
                    st + "_(X,Y,\"t**\"),not " + equalsName + "_(Y,X,\"t**\") .\n");
