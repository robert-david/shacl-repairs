...
public class RepairGenerator {
    ...
    static Set<String> finiteDomainProperties = new HashSet();
    ...
    private static void processShapes(List<Shape> shapes) {
        ...
        SHACLData.get().getDataFacts().add(
                "new(1.." + RepairData.get().getFiniteLimit() + ") .\n");
        ...
        if (path instanceof SimplePath) {
            ...
            for (Path pathEl : seq.getSequence()) {
                if (pathEl instanceof SimplePath || pathEl instanceof InversePath
                        || pathEl instanceof ZeroOrMorePath) {
                ...
                if (pathEl instanceof ZeroOrMorePath) {
                    ...
                    createZeroOrMorePathRules(getPathName(pathEl));
                }
            }
            ...
            for (Path pathEl : seq.getSequence()) {
                if (pathEl instanceof SimplePath) {
                    ...
                    String repairChoice =
                            pathName + "_star_(X" + (count >= 0 ? count : "") + ",X" + (count >= 0 ? count : "") + ",\"t*\"):-" + rule + " .\n";
        ...
        if (path instanceof SimplePath || path instanceof SequencePath) {
            ...
            if (minCount > 0) {
                ...
                for (int i = 1; i <= minCount; i++) {
                    ...
                    RepairData.get().getRepairRules().add(
                            "0 {" + firstPath + "_(X,Y,\"t\")} " + i +
                                    ":-choose(" + shapeName + ",X," + firstPath + "," + i + "),new(Y) .\n");

                    RepairData.get().getProgramConstraints().add(
                            ":-" + firstPath + "_(X,Y,\"t\")," + firstPath + "_(Z,Y,\"t\"),new(Y),X!=Z .\n");

                    finiteDomainProperties.add(firstPath);
                }
            ...
            if (minCount > 0) {
                ...
                for (int i = 1; i <= minCount; i++) {
                    ...
                    RepairData.get().getRepairRules().add(
                            "0 {" + firstPath + "_inv_(X,Y,\"t\")} " + i +
                                    ":-choose(" + shapeName + ",X," + firstPath + "," + i + "),new(Y) .\n");

                    RepairData.get().getProgramConstraints().add(
                            ":-" + firstPath + "_inv_(X,Y,\"t\")," + firstPath + "_inv_(Z,Y,\"t\"),new(Y),X!=Z .\n");

                    finiteDomainProperties.add(firstPath + "_inv");
                }
    ...
    private static void getUniversalRules(String shapeName, Path path, String st, Set<ConstraintComponent> valueNodeConstraints) {
        ...
        if (path instanceof SimplePath || path instanceof SequencePath) {
            ...
            RepairData.get().getRepairRules().add(
                    "0 {" + firstPath + "_(X,Y,\"t\")} " + 1 +
                            ":-choose(" + shapeName + ",X," + firstPath + "," + 1 + "),new(Y) .\n");
        ...
        for (int pI = 0; pI < seq.getSequence().size(); pI++) {
            ...
            if (pI < (seq.getSequence().size() - 1)) {
                ...
                if (seq.getSequence().get(pI) instanceof SimplePath) {
                    ...
                    RepairData.get().getRepairRules().add(
                            "0 {" + nextPath + "_(X" + currentPathIndex + ",Y,\"t\")} " + 1 +
                                    ":-choose(" + propertyPathName + ",X" + currentPathIndex + "," + nextPath + "," + 1 + "),new(Y) .\n");

                    RepairData.get().getProgramConstraints().add(
                            ":-" + nextPath + "_(X,Y,\"t\")," + nextPath + "_(Z,Y,\"t\"),new(Y),X!=Z .\n");

                    finiteDomainProperties.add(nextPath);
                    ...
                    RepairData.get().getRepairRules().add(
                            "0 {" + nextPath + "_inv_(X" + currentPathIndex + ",Y,\"t\")} " + 1 +
                                    ":-choose(" + propertyPathName + ",X" + currentPathIndex + "," + nextPath + "_inv_," + 1 + "),new(Y) .\n");
                    ...
                    // shifted to annotations rules for st properties, because universal rules do not generate this rule here
    ...
    private static void createZeroOrMorePathRules(String pathName) {

        RepairData.get().getAnnotationRules().add(pathName + "_star_(X,Y,\"t*\"):-" + pathName + "_(X,Y,\"t*\") .\n");
        RepairData.get().getAnnotationRules().add(
                pathName + "_star_(X,Z,\"t*\"):-" + pathName + "_star_(X,Y,\"t*\")," + pathName + "_(Y,Z,\"t*\") .\n");

        RepairData.get().getRepairRules().add(
                pathName + "_star_(X,Y,\"f\");" + pathName + "_(Y,Z,\"f\"):-" +
                        pathName + "_star_(X,Z,\"f\")," + pathName + "_star_(X,Y,\"t*\")," + pathName + "_(Y,Z,\"t*\"),X!=Y .\n");
        RepairData.get().getRepairRules().add(
                pathName + "_(X,Y,\"f\"):-" +
        ...
        RepairData.get().getProgramConstraints().add(
                ":-" + pathName + "_star_(X,Y,\"t\")," + pathName + "_star_(X,Y,\"f\") .\n");
        RepairData.get().getProgramConstraints().add(
                ":-" + pathName + "_star_(X,X,\"f\") .\n");
    }
    ...
    private static void createFiniteDomainConstraints() {

        List<String> pList = new ArrayList(finiteDomainProperties);
        for (int i = 0; i < pList.size(); i++) {
            String p1 = pList.get(i);
            for (int j = i + 1; j < pList.size(); j++) {
                String p2 = pList.get(j);
                RepairData.get().getProgramConstraints().add(
                        ":-" + p1 + "_(X,Y,\"t\")," + p2 + "_(Z,Y,\"t\"),new(Y) .\n");
            }
        }
    }
}
